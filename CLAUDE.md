...
**Four attempts to fix `agent_reasoning`'s empty-per-step problem — the first three failed differently, the fourth worked:**
...
3. A fully separate, tools-disabled call before the tool-calling turn (`call_llm_plain()`, no `tools` param at all), but reusing the SAME growing `messages` list the tool-calling turn uses (already containing real prior `tool_calls` entries) → the model attempted a tool call anyway, hallucinating a tool name that doesn't exist in this codebase (`ncbi_gene`), rejected by Groq (`Tool choice is none, but model called a tool`).
4. **WORKED.** Same "fully separate, tools-disabled call" idea as attempt 3, but with the key fix: the reasoning call (`_get_step_reasoning()` in `app/agent/investigation_loop.py`) gets a FRESH, minimal messages list every time (system + one user question, built from a plain-text progress summary) — never the shared tool-calling `messages` list, so the model never sees a tool-calling-shaped turn in this call at all, regardless of how many tools it already called earlier in the run. The tool-calling turn itself was also simplified to ask for nothing but the tool call ("Now call the appropriate tool.") — no reasoning requested there anymore, removed from `SYSTEM_PROMPT_TEMPLATE` entirely.

**Verified live, real: 3 runs against real SOD1 data through `POST /agent/investigate/1`, 10/10 steps returned real, substantive, non-empty `agent_reasoning`** (2 steps in run 1 — the model chose to stop earlier that run, not a reasoning failure — 4 each in runs 2-3). Example (run 2, step 1): *"I will assess how ALS‑associated SOD1 mutations disrupt metal binding and promote protein aggregation in motor‑neuron models, because these alterations are central to the toxic gain‑of‑function mechanism driving neurodegeneration."* Costs one extra real LLM call per step (a separate reasoning call before each tool-calling call) — accepted tradeoff for a working per-step audit trail.

One real, non-obvious pitfall hit while verifying this, worth remembering: the first live test after writing this code showed `agent_reasoning` empty at every step again, looking like a 4th failure — it was a stale, non-`--reload` uvicorn process still running the OLD code from before the edit, not a real failure. Confirmed by isolated testing (19/19 real, non-empty responses to the exact real prompts used at every step, run outside the server) and by actually restarting the server before re-testing. If a code change here "doesn't seem to take effect," check whether the server actually reloaded before concluding the code is wrong.
